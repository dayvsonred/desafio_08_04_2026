...
resource "aws_iam_role" "lambda_role" {
  name = "${var.project_name}-classify-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "${var.project_name}-classify-lambda-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = data.aws_dynamodb_table.categories.arn
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = data.aws_sqs_queue.classification.arn
      },
      {
        Effect = "Allow"
        Action = [
        ...
        Resource = [
          data.aws_sqs_queue.processing.arn,
...
resource "aws_lambda_function" "classify_complaint" {
  function_name = "${var.project_name}-classify-complaint"
  role          = aws_iam_role.lambda_role.arn
  handler       = "ClassifyComplaint.Function::ClassifyComplaint.Function.Function::FunctionHandler"
  runtime       = "dotnet8"
  timeout       = 60
  memory_size   = 512
...
resource "aws_lambda_event_source_mapping" "classification_queue_mapping" {
  event_source_arn        = data.aws_sqs_queue.classification.arn
